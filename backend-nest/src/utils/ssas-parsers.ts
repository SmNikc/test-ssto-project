// backend-nest/src/utils/ssas-parsers.ts

// ===========================
// 1) Ваши парсеры дат (как есть)
// ===========================
...
// ===========================
// 2) Парсеры SSAS-сообщений (добавлено)
// ===========================
...
// DMS координаты: 59°56'30"N  030°18'20"E  (секунды могут отсутствовать)
const RE_DMS_LAT = /(\d{1,2})[°\s]\s*(\d{1,2})(?:['’]\s*(\d{1,2})?)?\s*([NS])/i;
const RE_DMS_LON = /(\d{1,3})[°\s]\s*(\d{1,2})(?:['’]\s*(\d{1,2})?)?\s*([EW])/i;
...
/**
 * Парсинг письма SSAS: MMSI, классификация (TEST/ALERT), координаты (DMS).
 */
export function parseEmail(subject: string, body: string): ParsedSsas {
  const out: ParsedSsas = {};

  // MMSI из темы или тела
...
/**
 * Парсинг «сырого» текстового payload SSAS: MMSI/HEXID + классификация.
 */
