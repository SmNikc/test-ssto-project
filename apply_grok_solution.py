...
# -*- coding: utf-8 -*-
"""
Применение полного решения GROK для системы ССТО
Заменяет текущий index_14_36.html на исправленную версию
Путь: C:\Projects\test-ssto-project\apply_grok_solution.py
"""
...
def apply_grok_solution():
    """Применяет решение GROK с полным резервированием"""
    
    # Пути к файлам
    project_dir = Path(r'C:\Projects\test-ssto-project\frontend-static')
    current_file = project_dir / 'index_14_36.html'
    fixed_file = project_dir / 'index_fixed.html'
    
    print("=" * 70)
    print("ПРИМЕНЕНИЕ РЕШЕНИЯ GROK ДЛЯ СИСТЕМЫ ССТО")
    print("=" * 70)
    
    # 1. Создаем резервную копию текущего файла
    if current_file.exists():
        backup_name = f"index_14_36.html.backup_before_grok_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        backup_file = project_dir / backup_name
        shutil.copy2(current_file, backup_file)
        print(f"✅ Создана резервная копия: {backup_name}")
    
    # 2. Создаем файл index_fixed.html с решением GROK
    print("\n📝 Создание исправленного файла index_fixed.html...")
    
    # HTML код из GROK (полная версия)
    grok_html = '''<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    fixed_file.write_text(grok_html, encoding='utf-8')
    print(f"✅ Создан файл: index_fixed.html")
    
    # 3. Предлагаем заменить текущий файл
    print("\n" + "=" * 70)
    print("ГОТОВО! Теперь у вас есть два файла:")
    print(f"1. {current_file.name} - текущий (возможно с дублями)")
    print(f"2. index_fixed.html - исправленный от GROK")
    print("\n" + "=" * 70)
    print("ЧТО ДЕЛАТЬ ДАЛЬШЕ:")
    print("1. Откройте index_fixed.html в браузере")
    print("2. Проверьте, что всё работает")
    print("3. Если всё ОК - переименуйте:")
    print("   - index_14_36.html → index_14_36.html.old")
    print("   - index_fixed.html → index_14_36.html")
